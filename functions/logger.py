...
from authentication.models import UserIp, HistoryLog
...
class RequestMessageMiddleware(MiddlewareMixin, ModelFunc):
	def process_request(self, request):
		...
		if "static" not in path and "media" not in path:
			method = request.method
			date = dt.datetime.now()
			time = dt.datetime.now()
			model = HistoryLog
			last_record = model.objects.create(date=date, time=time, ip=ip, action=method, web_address=path,
			                                   user_name_id=user_id, session=session, csrftoken=csrftoken)
	...
	def process_response(self, request, response):
		...
		valid_pages = ["/404", "/500", "/400", "/300", "/", "/register/", "/login/", "/register_validation/",
		               "/password_reset/", "/password_reset/done/", "/password_change/", "/password_change/done/",
		               "/reset/done/", "/logout/"]
